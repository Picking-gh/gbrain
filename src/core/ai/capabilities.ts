...
export function getProviderCapabilities(
  ...
  // `cache_mode` config ({"<provider>":"auto"}) lifts the recipe default:
  // providers with automatic prefix caching (OpenAI, DeepSeek, Gemini, Groq,
  // LiteLLM-proxied backends, etc.) report supportsPromptCaching=true so
  // doctor + model-config skip the cost warn. See ai/cache-mode.ts. This does
  // NOT affect gateway cache_control injection — that's gated on the recipe's
  // supports_prompt_cache field.
